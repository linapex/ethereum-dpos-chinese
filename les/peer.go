...
const maxResponseErrors = 50 //允许的无效响应数（使协议不那么脆弱，但仍然避免垃圾邮件）
...
//sendproofsv2发送一批merkle校对，与请求的校对相对应。

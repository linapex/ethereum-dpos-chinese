...
type TxType uint8
const (
Binary         TxType = iota //之前的转账或合同调剂交易
RegCandidate                 //注册为候选人
UnregCandidate               //注成为候选人
Delegate                     //用户为候选人投票
UnDelegate                   //销售发票（授权委托书）
)
...
func (tx *Transaction) Validate() error {
	if tx.Type() != Binary {
		...
		if tx.To() == nil && tx.Type() != RegCandidate && tx.Type() != UnregCandidate {

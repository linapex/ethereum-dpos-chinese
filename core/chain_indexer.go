...
//addknownsectionHead将新的分区头标记为已知/已处理（如果更新）
//比已知最好的部门主管
...
//REORG到公共祖先（可能不存在于光同步模式中，请跳过REORG）
//托多（卡拉贝尔，兹费尔福迪）：这似乎有点脆弱，我们能明确地检测到这个病例吗？

//TODO（karalabe）：此操作成本高昂，可能会阻塞，导致事件系统
//可能也会锁定。我们需要以不同的方式处理。

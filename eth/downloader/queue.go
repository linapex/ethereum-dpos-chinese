...
receiptTaskQueue *prque.Prque                  //[ETH/63]标题的优先级队列，用于获取收据
...
//ReserveBodies为给定的对等端保留一组正文提取，跳过任何
...
//ReserveReceipts为给定的对等方保留一组收据提取，跳过
...
//ReserveHeaders为给定的对等端保留一组数据下载操作，
...
//将所有跳过的邮件头合并回
	...
	if progress {
//wake waitresults，resultcache已修改
		q.active.Signal()
	}
//汇编并返回块下载请求
	if len(send) == 0 {

...
headerStarted  time.Time //上次头提取开始时的时间实例
blockStarted   time.Time //上次块（体）提取开始时的时间实例
receiptStarted time.Time //上次接收提取开始时的时间实例
...
//setblocksidle将对等机设置为空闲，允许它执行新的块检索
//请求。它的估计块检索吞吐量用测量的更新。
//刚才。
...
//setbodiesidle将对等机设置为空闲，允许它执行块体检索。

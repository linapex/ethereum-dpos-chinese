...
qosTuningImpact  = 0.25 //新的优化目标对上一个值的影响
...
fsHeaderCheckFrequency = 100             //快速同步期间下载邮件头的验证频率
fsHeaderSafetyNet      = 2048            //检测到链冲突时要丢弃的头数
fsHeaderForceVerify    = 24              //要在接受透视之前和之后验证的标题数
fsHeaderContCheck      = 3 * time.Second //状态下载期间检查头继续的时间间隔
...
//状态
synchroniseMock func(id string, hash common.Hash) error //测试过程中的同步替换
	synchronising   int32
	notified        int32
	committed       int32
...
headerProcCh  chan []*types.Header //[ETH/62]为头处理器提供新任务的通道
...
receiptFetchHook func([]*types.Header) //开始获取收据时调用的方法
...
//产卵同步运行d.process和所有给定的提取函数以在中完成
...
//不要等待正在运行的下载Goroutines完成。这个方法应该是
//从下载程序内部取消下载时使用。
...
//如果每一个标题都是已知的，甚至是未来的标题，那么同行们就会直接说谎。
...
header := d.lightchain.GetHeaderByHash(headers[0].Hash()) //独立于同步模式，头文件肯定存在

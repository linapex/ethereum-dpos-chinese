...
timeout  time.Duration              //完成此操作的最大往返时间
...
//runstatesync运行状态同步，直到完成或另一个根目录为止。
//请求将哈希切换到。
...
//statesync计划下载特定state trie定义的请求
//通过给定的状态根。
type stateSync struct {
d *Downloader //用于访问和管理当前对等集的下载程序实例

sched  *trie.Sync                 //状态trie-sync计划程序定义任务

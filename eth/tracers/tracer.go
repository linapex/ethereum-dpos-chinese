...
//biginegerjs是https://github.com/peterrolson/bigineger.js的小型版本。
...
//makeslice将具有给定类型的不安全内存指针转换为go字节
//切片。
//
//注意，返回的切片使用与输入参数相同的内存区域。
//如果这些是Duktape堆栈项，则弹出它们**将**成为切片
//内容更改。
...
//popsicle从javascript堆栈中弹出一个缓冲区，并将其作为一个切片返回。
...
//pushbigint在虚拟机中创建一个javascript biginger。
...
//opwrapper提供了一个围绕操作码的javascript包装器。
type opWrapper struct {
	op vm.OpCode
}

//PushObject组装包装可交换操作码的JSVM对象并将其推送
//到VM堆栈。
...
//memory wrapper提供了一个围绕vm.memory的javascript包装器。
type memoryWrapper struct {
	memory *vm.Memory
}

//slice以字节片的形式返回请求的内存范围。
func (mw *memoryWrapper) slice(begin, end int64) []byte {
	if mw.memory.Len() < int(end) {
//托多（卡拉拉比）：我们不能把球从内往外扔。围棋
//运行时崩溃https://github.com/golang/go/issues/15639。
...
//pushobject组装包装可交换内存的JSVM对象并将其推送
//到VM堆栈。
func (mw *memoryWrapper) pushObject(vm *duktape.Context) {
	obj := vm.PushObject()

//生成接受两个整数并返回缓冲区的“slice”方法
...
//生成“getuint”方法，该方法接受int并返回bigint
	vm.PushGoFunction(func(ctx *duktape.Context) int {
...
//stack wrapper提供了一个围绕vm.stack的javascript包装器。
...
//PushObject组装包装可交换堆栈的JSVM对象并将其推送
//到VM堆栈。
...
//dbwrapper提供了一个围绕vm.database的javascript包装器。
type dbWrapper struct {
	db vm.StateDB
}

//pushobject组装包装可交换数据库的JSVM对象并将其推送
//到VM堆栈。
func (dw *dbWrapper) pushObject(vm *duktape.Context) {
	obj := vm.PushObject()

//推送statedb.getbalance的包装
	vm.PushGoFunction(func(ctx *duktape.Context) int {
...
//ContractWrapper提供了一个围绕vm.contract的javascript包装器。
type contractWrapper struct {
	contract *vm.Contract
}

//PushObject组装包装可交换协定的JSVM对象并将其推送
//到VM堆栈。
func (cw *contractWrapper) pushObject(vm *duktape.Context) {
	obj := vm.PushObject()

//为contract.caller推送包装
	vm.PushGoFunction(func(ctx *duktape.Context) int {
...
//跟踪程序提供跟踪程序的实现，该跟踪程序评估JavaScript
//每个VM执行步骤的函数。
type Tracer struct {
inited bool //标记上下文是否已从EVM初始化

vm *duktape.Context //javascript虚拟机实例

tracerObject int //跟踪程序javascript对象的堆栈索引
stateObject  int //要从中提取参数的全局状态的堆栈索引

opWrapper       *opWrapper       //围绕VM操作码的包装器
stackWrapper    *stackWrapper    //围绕VM堆栈包装
memoryWrapper   *memoryWrapper   //包装虚拟机内存
contractWrapper *contractWrapper //包装合同对象
dbWrapper       *dbWrapper       //包装虚拟机环境

pcValue    *uint   //由日志访问器包装的可交换PC值
gasValue   *uint   //由日志访问器包装的可交换气体值
costValue  *uint   //日志访问器包装的可交换成本值
depthValue *uint   //日志访问器包装的可交换深度值
errorValue *string //日志访问器包装的可交换错误值

ctx map[string]interface{} //在执行过程中收集的事务上下文
err error                  //如果发生错误

interrupt uint32 //信号执行中断的原子标志
reason    error  //中断的文字原因
}

//New实例化新的跟踪程序实例。代码指定了一个javascript代码段，
//它必须计算为返回带有“step”、“fault”的对象的表达式
//和“result”函数。
func New(code string) (*Tracer, error) {
//按名称解析任何跟踪程序并组装跟踪程序对象

...
//mustasset与asset类似，但在asset返回错误时会出现恐慌。
//它简化了全局变量的安全初始化。
...
//mustassetstring就像assetstring，但当asset返回
//错误。它简化了全局变量的安全初始化。
func MustAssetString(name string) string {
	return string(MustAsset(name))
}

//asset info加载并返回给定名称的资产信息。
//如果找不到资产或
//无法加载。
...
//assetdigest返回具有给定名称的文件摘要。它返回一个
//如果找不到资产或无法加载摘要，则出错。
...
//assetdir返回某个
//通过go bindata嵌入到文件中的目录。
//例如，如果对数据/运行go bindata。数据包含
//以下层次结构：
//数据/
//英尺·txt
//IMG/
//A.PNG
//B.PNG
//然后assetdir（“data”）将返回[]字符串“foo.txt”，“img”，
//assetdir（“data/img”）将返回[]字符串“a.png”、“b.png”，
//assetdir（“foo.txt”）和assetdir（“notexist”）将返回错误，以及
//assetdir（“”）将返回[]字符串“data”。

...
/*
func（api*publicethereumpi）etherbase（）（common.address，错误）
 返回api.e.etherbase（）。
*/
...
/*
func（api*publicethereumpi）coinbase（）（common.address，错误）
 返回api.etherbase（）。
*/
...
//用给定数量的线程启动矿工。如果线程为零，则为数字
//启动的工作线程数等于
//这个过程。如果挖掘已在运行，则此方法将调整
//允许使用和更新事务所需的最低价格的线程
//池。
...
//setetherbase设置矿工的etherbase
/*
func（api*privateminerapi）setetherbase（etherbase common.address）bool_
 api.e.setetherbase（以太网）
 返回真
*/
...
//privateAdminAPI是以太坊完整节点相关API的集合。
//在私有管理终结点上公开。
...
//newpublicDebugAPI为完整节点创建新的API定义-
//以太坊服务的相关公共调试方法。
...
//GetBadBlocks返回客户端在网络上看到的最后一个“坏块”的列表
//并将其作为块哈希的JSON列表返回

...
func readprotocolhandshake（rw msgreader，our*protochandshake）（*protochandshake，error）
 ...
 如果msg.code==discmsg
  //根据协议握手有效之前断开连接
  //spec，如果posthashake检查失败，我们将自己发送它。
...
//并验证加密握手是否“有效”以及
//远程端实际上提供了正确的公钥。
func（t*rlpx）doenchandshake（prv*ecdsa.privatekey，dial*discover.node）（discover.nodeid，错误）
 var
  证券交易秘密
  错误率
 ）
...
类型enchandshake结构
 ...
 remotepub*ecies.publickey//远程pubk
...
func（h*enchandshake）staticsharedsecret（prv*ecdsa.privatekey）（[]字节，错误）
 返回ecies.importecdsa（prv）.generateshared（h.remotepub、ssklen、ssklen）
}
...
func（h*enchandshake）makeauthmsg（prv*ecdsa.privatekey）（*authmsgv4，错误）
 rpub，错误：=h.remoteid.pubkey（）
 如果犯错！= nIL{
  返回nil，fmt.errorf（“错误的remoteid:%v”，err）
 }
 h.remotepub=ecies.importecdsapublic（rpub）
 //生成随机发起程序nonce。
 h.initonce=生成（[]字节，shalen）
 如果，错误：=rand.read（h.initnoce）；错误！= nIL{
  返回零
 }
 //为ECDH生成随机密钥对to。
...
func（h*enchandshake）handleauthmsg（msg*authmsgv4，prv*ecdsa.privatekey）错误
 ...
 h.remoteid=msg.initiatorSubkey
 rpub，错误：=h.remoteid.pubkey（）
 如果犯错！= nIL{
  返回fmt.errorf（“错误的remoteid:%v”，err）
 }
 h.remotepub=ecies.importecdsapublic（rpub）
...
func（msg*authmsgv4）sealplain（h*enchandshake）（[]字节，错误）
 ...
 buf[n]=0//令牌标志
 返回ecies.encrypt（rand.reader，h.remotepub，buf，nil，nil）
}
...
func（msg*authrespv4）sealplain（hs*enchandshake）（[]字节，错误）
 ...
 复制（buf[n:]，msg.nonce[：]）
 返回ecies.encrypt（rand.reader、hs.remotepub、buf、nil、nil）
}
...
func sealeip8（msg interface，h*enchandshake）（[]字节，错误）
 ...
 enc，err：=ecies.encrypt（rand.reader，h.remotepub，buf.bytes（），nil，前缀）

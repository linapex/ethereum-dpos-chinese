...
//添加发送者，就好像它刚刚打了我们。重新验证应替换中的最后一个节点
//它的桶如果没有反应。再次验证以确保
...
//这是查找测试的测试网络。
//通过以随机节点ID为目标运行testnet.mine来获取节点。

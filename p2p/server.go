...
//节点表
	...
	if srv.DiscoveryV5 {
		...
		if sconn != nil {
ntab, err = discv5.ListenUDP(srv.PrivateKey, sconn, realaddr, "", srv.NetRestrict) //服务器节点数据库）
		} else {
ntab, err = discv5.ListenUDP(srv.PrivateKey, conn, realaddr, "", srv.NetRestrict) //服务器节点数据库）
		}
		if err != nil {
			return err
		}

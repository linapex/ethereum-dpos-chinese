...
//*
//回答：设备要求计算机显示管脚矩阵，并等待使用此矩阵方案编码的管脚。
//@下一个Pinmatrixack
//下次取消
...
//*
//请求：如果未使用resetdevice备份设备种子，请执行备份
//@下一步按钮请求
...
//*
//请求：启动恢复工作流，要求用户输入特定的助记词
//用于安全恢复设备，即使在不受信任的计算机上。
//@下一个字请求
...
//*
//请求：事务的估计大小
//它的行为与signtx完全相同，这意味着它可以使用txrequest请求
//此调用是非阻塞的（除了可能的密码请求以解锁种子）
//“下一个TXSIZE”
//下一次失败
...
//*
//请求：简化事务签名
//此方法不支持流式处理，因此在输入和输出数量上存在硬件限制。
//如果成功，将使用txrequest消息返回结果。
//@下一个密码请求
//@下一个PinMatrixRequest
//下一个TXT请求
//下一次失败
...
//*
//响应：设备请求签署事务的信息或返回最后一个结果
//如果设置了请求_索引，则设备等待txack消息（根据请求_类型填写字段）
//如果设置了签名索引，“签名”包含签名索引输入的签名输入
//预兆信号
//@上一个simplesigntx
//普雷克
...
//*
//请求：请求设备签署事务
//从协议的角度来看，所有字段都是可选的。如果缺少，则每个字段默认为值“0”。
//注意：第一个最多1024字节的数据必须作为此消息的一部分进行传输。
//@下一个密码请求
//@下一个PinMatrixRequest
//@下一个以太坊MtxRequest
//下一次失败
...
//*
//响应：设备从事务负载请求更多数据，或返回签名。
//如果设置了数据长度，设备将等待更多字节的有效负载。
//否则，签名字段包含计算出的事务签名。所有三个字段都将显示。
//@prev以太坊GNTX
//@下一个以太坊
...
//*
//请求：要求设备删除其固件（以便可以通过firmwareupload进行更换）
//下一个成功
//@下一个固件请求
//下一次失败
...
//*
//响应：设备将内存发送回
//@prev debuglinkmemoryread（上一个debuglinkmemoryread）

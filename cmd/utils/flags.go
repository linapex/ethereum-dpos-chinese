...
//“github.com/ethereum/go-ethereum/consension”网站
//“github.com/ethereum/go-ethereum/consension/clique”
//“github.com/ethereum/go-ethereum/consultance/ethash”
...
//set node key从set命令行标志创建一个节点键，或者加载它
//从文件或作为指定的十六进制值。如果没有提供任何标志，则
//方法返回nil，将生成一个临时键。
...
//setbootstrapnodesv5从命令行创建引导节点列表
//标记，如果未指定，则恢复为预配置的标记。
...
//setlistenaddress从set命令创建TCP侦听地址字符串
//行标志。
...
//makedatabasehandles引发每个进程允许的文件句柄数
//并返回要分配给数据库的津贴的一半。
...
return limit / 2 //留下一半用于网络和其他东西
}

//makeaddress转换直接指定为十六进制编码字符串或
//内部帐户表示形式的密钥存储中的密钥索引。
...
//
//除非使用--nodiscover显式禁用它，否则请注意显式指定
//--v5disc overrides--nodiscover，在这种情况下，后者只禁用v4发现
...
//checkExclusive验证提供的标志只有一个实例
//
//进一步专业化。
func checkExclusive(ctx *cli.Context, args ...interface{}) {
	set := make([]string, 0, 1)
	for i := 0; i < len(args); i++ {
//确保下一个参数是标志，如果未设置，则跳过。
		flag, ok := args[i].(cli.Flag)
		if !ok {
			panic(fmt.Sprintf("invalid argument, not cli.Flag type: %T", args[i]))
		}
//检查next参数是否扩展当前参数，如果扩展，则扩展其名称
		name := flag.GetName()
...
//setshhconfig将与shh相关的命令行标志应用于配置。
...
//TODO（FJL）：将trie缓存生成移动到配置中
	if gen := ctx.GlobalInt(TrieCacheGenFlag.Name); gen > 0 {
		state.MaxTrieCacheGen = uint16(gen)
	}
}

//setdashboardconfig将与仪表板相关的命令行标志应用于配置。
...
//registerethservice将以太坊客户端添加到堆栈中。
...
//RegisterDashboardService将仪表板添加到堆栈中。
...
//SetupNetwork为主网络或某些测试网络配置系统。
func SetupNetwork(ctx *cli.Context) {
//TODO（FJL）：将目标气体限制移动到配置中
...
//makechaindatabase使用传递给客户机的标志打开一个leveldb，如果失败，它将很难崩溃。
...
//makeconsolepreloads检索控制台javascript的绝对路径
//启动前要预加载的脚本。
...
//migrateflags在设置本地标志时从该标志设置全局标志。
//这是一个临时函数，用于将旧命令/标志迁移到
//新格式。
//
//例如，geth account new--keystore/tmp/mykeystore--lightkdf
//
//在用以下方法调用此方法后等效：
//
//geth--keystore/tmp/mykeystore--lightkdf新帐户
//
//这允许使用现有的配置功能。
//当迁移所有标志时，可以删除此函数，并且
//必须更改使用本地标志的配置功能
